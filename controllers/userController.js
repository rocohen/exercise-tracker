...
const createExercise = async (req, res) => {
  ...
  date === 'Invalid Date' ? (date = new Date().toDateString()) : date;

  if (!_id || !description || !duration) {
    res.status(400).json({
      error:
        'Missing Data. Please fill the required fields in the form and try again.',
    });
  } else {
...
const getUserLogs = async (req, res) => {
  ...
  const startDate =
    from && new Date(from).toDateString() !== 'Invalid Date'
      ? new Date(from).toDateString()
      : '';
  const endDate =
    to && new Date(to).toDateString() !== 'Invalid Date'
      ? new Date(to).toDateString()
      : '';
  const user = await User.findById(_id).exec();

  if (from && to) {
    const exerclog = await Exercise.find({ user: _id })
    ...
    const exerclog = await Exercise.find(
      { user: _id },
